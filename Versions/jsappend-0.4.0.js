...
(function(){
	...
	jsAppend.resolveStringPath = function(x, y){
		...
		for(var i in doms){
			...
			// If dom is based on its className
				...
				if(doms[i].match(/\[\d+\]$/i)){
					// Get class name
					var className = doms[i].substr(1).replace(/\[\d+\]$/i,"");
					// Get element number
				...
				// If we deal with a collection
					...
					//Try to enter path
					if(path.getElementsByClassName( doms[i].substr(1) ) ){
						path = path.getElementsByClassName( doms[i].substr(1) );
						collection = true;
					//Path dont exist
					}else{
						path = null;
	...
	jsAppend.createJsAppendElement = function(x){
		...
		var newJsAppendElement = new this.JsAppendElement();

		// If parameter is a type
		if(typeof x == "string")
			newJsAppendElement.element = document.createElement(x);

		// If parameter is an element
		else if(x && x.tagName && x.nodeName && x.ownerDocument && x.removeAttribute)
			newJsAppendElement.element = x;
	...
	jsAppend.JsAppendElement = (function(){
		...
		JsAppendElement.prototype.fade = {
		...
		JsAppendElement.prototype.fadeInStart = function(speed, callback){
			...
			if(this.fade.anim)
				this.fade.anim.stop();
			this.fade.anim = jsAppend.animate(
				function(x){ if(!e.element)return; e.element.style.opacity = x; }
			, 0, 1, speed,
				function(){ if(!e.element)return; e.element.style.opacity = 1; callback(); }
			);
			return this;
		}
		...
		JsAppendElement.prototype.fadeOutStart = function(speed, callback){
			...
			this.fade.anim = jsAppend.animate(
				function(x){ if(!e.element)return; e.element.style.opacity = x; }
			, 1, 0, speed,
				function(){ if(!e.element)return; e.hide(); e.element.style.opacity = 1; callback(); }
			);
			return this;
		}
